...
public class ModifiedAES {
        ...
        public static void main (String[] args) {
            ...
            String filePath=aes.browseFile();
            String fileName = filePath;
            String lines = aes.readFileToString(fileName);

            System.out.println("AES program is running.......");
            ...
            aes.showResults(data, key, encryptedText, encryptedHexText, unencryptedText, DNAcode, decodedmessage);
        ...
        public byte[] encrypt(byte[] plain) {
            ...
            // SubBytes(state)
            byte[] lastSBox=generateSBox(Ker);
        ...
        public byte[] decrypt(byte[] cipher) {
            ...
            for (int r = 1; r < numRounds; r++) {
                ...
                // InvShiftRows(state)
                int shift[]=shiftRowsGenerator(Kdr);
                for (i = 0; i < BLOCK_SIZE; i++) {
                    row = i % COL_SIZE;
                ...
                // InvSubBytes(state)
                byte[] generatedSBox=generateDSBox(Kdr);
            ...
            // InvSubBytes(state)
            byte[] lastSBox=generateDSBox(Kdr);
        ...
        public void showResults(String data, String key, String encryptedText, String encryptedHexText, String unencryptedText, String DNAcode, String decodedmessage) {
            System.out.println("\nText from file: ");
            ...
            System.out.println("\n");
            System.out.println("DNA encrypted text: "+DNAcode);
            System.out.println("\n");
            System.out.println("DNA decrypted text: "+decodedmessage);
            //System.out.println("\n");
            //System.out.println("HEX Encrypted Text: " + encryptedHexText);
            System.out.println("\n");
        ...
        //generate s-box
        public static byte[] generateSBox(byte[] roundKey) {

            long shiftValue = roundKey[0] ^ roundKey[roundKey.length - 1];
            //long shift = (long) shiftValue;
            long shift=-3;
            if(shift<0){
                shift=-(shift);
            }
    ...
    public static byte[] generateDSBox(byte[] roundKey) {
        ...
        byte[] newSBox = new byte[256];
        for (long i = 0; i < DSBox.length; i++) {
            long newIndex = (i + shift) % DSBox.length;
            newSBox[(int) newIndex] = DSBox[(int) i];
        }
        return newSBox;
    }

    //shiftrows array generator
    public static int[] shiftRowsGenerator(byte[] roundKey) {
        int[] shiftingValue = new int[4];
        int[][] keyMatrix = new int[4][4];

        // Convert roundKey from byte array to 2D integer matrix
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                keyMatrix[j][i] = roundKey[i * 4 + j] & 0xFF;
            }
        }

        // Get the diagonal values and compute the shifting values
        for (int i = 0; i < 4; i++) {
            int diagonalValue = keyMatrix[i][i];
            shiftingValue[i] = diagonalValue % 4;
        }
